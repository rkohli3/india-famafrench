...
Our present approach is to test our hypothesis, whether Fama/French factor portfolios can be applied to India, on a sample of 300 stocks 
...
- Filtering Fundamental data:
	...
	lag before this information is availble to public, we sort our portfolios on 1st July every month and rebalance every month for
	an equally weighted portfolio. We front fill any fundamentall data not availble for a particular quarter or year. We noticed that 
	the front filling was not extensive as ProwessDx proivided very exhaustive and comprehensive data for both BSE and NSE companies
	...
	out of which ony 1681 survived by March of 2017.
...
eleven months i.e :math:`t-12` to :math:`t-1`, excluding the most recent month as it is not an indeicator of momentum. The top 30% are considered as the Winners and bottom 30% are considered 
...
- For 2-D sorting, we use the intersection of `2 X 3` portfolios for Size and Value, `2 X 3` portfolios for Size and Momentum and `3 X 3` portfolios for Momentum and Value.
  The interesectional sorts uses 3 parameters
...
The methodology for **2-D** sorting for `Momentum` and `Size` is illustred below.
